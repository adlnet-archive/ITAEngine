...
{
	public interface IAssetHeuristicEngine
	{
		/**
		 * Generate assets scores based pastAsset usage patterns
		 **/
		void generateAssetScores(ICollection<string> assetTypes, IList<AssetRecomendation> pastAssets);
		
		/**
		 * Get the score based on heuristic engine analysis for the AssetType
		 **/
		int getScoreForType(string AssetType);
	}
}
